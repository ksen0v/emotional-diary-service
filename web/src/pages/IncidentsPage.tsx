...
//  2. Коэффициент дисциплины в полосе. В прототипе там только счётчики, но
//     число уже считается и приходит в totals, а «считаем, но не показываем» —
//     обратная сторона правила «экран не врёт». Решение Влада на шаге 11:
//     добавить сейчас и отступление назвать. Это дисциплина ПО ИНЦИДЕНТАМ —
//     доля соблюдённых среди закончившихся, — и она не то же самое, что
//     коэффициент дисциплины в дневнике, который считается по разметке
//     сделок (ТЗ 5.1). Поэтому подпись называет её полностью: два разных
//     числа под одним именем на двух экранах — худшее из возможного.
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flexGrow: 1, minHeight: 0 }}>
      ...
      >
        ...
        {/* Прочерк, а не 100%, пока ни один инцидент не закончился: доля
            соблюдённых среди нуля закончившихся — не «отлично», а «нечего
            измерять». */}
        <span style={{ fontSize: 13, color: 'var(--dim)' }}>
          Дисциплина по инцидентам{' '}
          <span className="mono" style={{ color: 'var(--fg)' }}>
            {totals.discipline_pct === null
              ? '—'
              : `${Math.round(totals.discipline_pct)}%`}
          </span>
        </span>
