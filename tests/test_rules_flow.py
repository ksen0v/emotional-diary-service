...
async def test_each_unfinished_thing_has_its_own_flag(
    app_client: httpx.AsyncClient,
) -> None:
    """У каждого «ещё не готово» свой флаг, а не отрицание общего.

    Урок шага 9 в виде теста. Тогда предупреждение про системные триггеры
    висело на `engine.active`: движок включился, флаг стал `true` — и
    предупреждение ушло вместе с ним, хотя триггеры не срабатывали.
...
async def test_preview_of_system_rule_uses_its_own_condition(
    ...
    assert out["human_text"] == (
        ...
        # Вторая развязка того же условия (ТЗ 4.4). Появилась в фразе на
        # шаге 11 — вместе с механикой, не раньше и не позже: до него
        # поздний тег не делал ничего, и обещать разбор прошлого дня было
        # бы тем самым враньём экрана, от которого лечился шаг 10.
...
async def test_sr1_declares_both_branches_of_the_tag(
    app_client: httpx.AsyncClient,
) -> None:
    """У SR-1 две развязки, и фраза называет обе (ТЗ 4.4).

    Обратная сторона предыдущего теста. Там правило не должно обещать
    лишнего, здесь — не должно умалчивать о том, что делает. До шага 11
    поздний тег не делал ничего, и молчание было честным; теперь он
    разбирает прошлый день, и правило, объявляющее только блокировку,
    описывало бы половину себя.

    Заодно это ответ на первый вопрос трейдера про поздний тег: «а если
    размечу вечером — обойдётся?». Обе половины стоят рядом именно поэтому.
    """
    await register(app_client)
    by_code = {r["system_code"]: r for r in (await rules(app_client))["rules"]}
    text = by_code["SR-1"]["human_text"]

    # Живая ветка: блокировка до конца дня.
    assert "заблокируется до конца торгового дня" in text
    # Поздняя ветка: блокировки нет, есть разбор прошлого дня.
    assert "после конца того дня" in text
    assert "блокировку не включает" in text
    assert "задним числом" in text
    # И предупреждения «поздний тег не проверяется» на карточке больше нет:
    # оно ушло вместе с флагом, когда механика появилась.
    assert by_code["SR-1"]["pending"] is None
