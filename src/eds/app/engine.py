...
async def run_day(
    ...
    *,
    ...
    if await refresh_lock(s, user_id, moment):
        out.breaches += 1
    return counters
...
async def refresh_lock(
    ...
    breached = await incidents.record_breach(
        s,
        user_id,
        lock,
        [OpenTrade(trade_id=r[0], symbol=r[1], open_time=r[2]) for r in rows],
        now,
    )
...
async def today_block(
    ...
    *,
    ...
    """
    moment = now or dt.datetime.now(dt.UTC)
    await refresh_lock(s, user_id, moment)
