...
METRICS: tuple[Metric, ...] = (
    ...
    Metric(
        ...
        min=Decimal("0.1"),
        max=Decimal("50"),
        hint="Сумма текущей серии убытков, а не всего дня.",
    ),
    # Две метрики, которые появятся вместе с Binance (ТЗ 4.2, Архитектура ч.1 §5.7):
    # у источника с открытыми позициями просадка считается честно, с учётом
    # незакрытого минуса — того самого момента тильта, который у TMM слепая зона.
    Metric(
